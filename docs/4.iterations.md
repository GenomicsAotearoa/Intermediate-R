...
!!! note "`while` loops"
    ...
    As seen above, `for` loops require a defined range for iteration. There are scenarios where we want an iteration to go for as long as required until some condition is reached. In this case, we can use the `while` loop. 
...
* **Use `map()`/`walk()` and variants to apply throwaway functions.** This is especially useful if you are dealing with multiple data sets and need to clean data in specific ways. This way, you can use one script that produces clean data via anonymous functions and save precious time (and mental energy!) to write and name functions that serve general use cases!
