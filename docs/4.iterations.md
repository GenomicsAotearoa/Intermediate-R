...
"Applies" a function along vector(s)/array(s) with more concise code. The iteration is happening in the background.
...
It is good practice to pre-assign vectors to store outputs of iterations (if desired). This ensures that R pre-allocates sufficient memory for the outputs. While you can assign an empty vector then grow the vector by iteratively adding elements per iteration, you may run out of memory if the vector grows too large. Furthermore, a preallocated output can be used as the object under iteration (as above).

If you think you will be processing a large number of iterations and need to store the output, consider coercing those into flat text formats (e.g., tab/comma separated values, strings, etc.) and writing it out to disk with a function that can open and append existing files (e.g., `readr::write_csv()` with `append = TRUE`). If you are going to do this, be mindful of the available disk space.
...
While iterations are helpful when you have a lot of data to go through, it can be computationally expensive. Try to reduce the work done within an iteration and use vectorised functions to achieve part of the task. Consider also if only part of data requires an iteration for analysis. If there are many steps that must be achieved using iteration, try to break those steps up into smaller iterations. When using iterations inside function, it is even more important to exit iterations as quickly as possible so as to not stall if things go wrong (recall "failing fast").

If you must iterate through a large amount of data, also consider using other control flow elements like `if-else` together with loop specific statements like `break` or `next` to make sure you are only computing on vectors that meet a certain threshold.

**Structural and analytical considerations**

How data is structured is often the most important consideration when deciding between using `for` loops and `map()`. If data and subsequent analyses are amenable to combinatorial methods like `combn()` or `expand.grid()`, we can make life easier by using the `map()` method. On the other hand, it is difficult (sometimes impossible) to coerce some data structures into "flat" tables without extensive modification (and code). For example, if the data is stored in a highly nested and complex manner, a `for` loop may be more appropriate, readable, and verbose. Some analyses are not amenable to "flattening", such as those that require updating objects in the global environment or is (conditionally) dependent on results from previous iterations. Remember, computation time is cheap compared to human time. Choose the best construct that will get the job done within hardware constraints.
