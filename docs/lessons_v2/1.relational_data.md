...
The four data frames have $\gt$ 4 thousand lines and 24 columns. We can choose to explore the results of the different joins using `View()`, but that might take awhile. Instead, let's use some fundamental properties of the data to deduce how the results differ. These properties are:
...
B ased on the information above, our data can be represented using a Venn diagram as below.

![venn](../theme_figures/relational_data_venn.png)
...
??? question "Why use `inner_join()` and `full_join()`? An anecdotal account."
    ...
    Let’s assume that we are interested in well-characterised genes and pathways. Multiple analyses should have produced concordant results for a subset of sequences (e.g., a query sequence with homologous to citrate synthase should be found regardless of annotation against KEGG, NCBI RefSeq, or UniProt databases), thereby reducing the chance of false positives or hits with low probability of homology. In this scenario, using `inner_join()` will combine those results based on your sequence ID, generating a table consisting only of results shared consistently across both analyses.
...
<span style="color:red">...Add scenario here...</span>

In cases where duplicated keys exist in either data frames used for joining, the default behaviour of mutating joins is to output the Cartesian product of both data frames based on their keys. That is to say, all combinations of the keys will be used to generate the newly joined table.

<span style="color:red">...Add resulting table here...</span>
