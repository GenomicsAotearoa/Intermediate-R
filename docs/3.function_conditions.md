...
It's not a secret that for as long you've used R, you've been using functions. Here, we're going to make them! Functions are, like all things in R, a type of object (implications discussed in a [later episode](#data-structures)). They are the "verbs" of the R programming language where, instead of storing data, functions store code that defines how it behaves and operates on other objects.
