...
Now modify the vectors 21 more times! How cumbersome! However, we know we can turn the calculations into a function, like so:
...
??? tip "Weak points of `diversity_index`"
    ...
    * `NA` handling only for Shannon's index, but not others (What if input vector contains missing values?)
...
To mitigate some of those problems, we might want to restrict how our function evaluates and processes input, as well as have the function provide useful error messages in case of problems. Here, two base R functions are useful:
...
!!! r-project ""
    ...
    ```r
    diversity_index <- function(abundance, index = "shannon", log.base = exp(1), na.rm = FALSE) {
      ...
      if (any(is.na(abundance)) & isFALSE(na.rm)) {
        stop("There are missing values and na.rm = FALSE!")
      }
      ...
      p_i <- abundance / sum(abundance, na.rm = na.rm)

      if (isTRUE(str_detect(index, "simpson"))) {
        # Simpson's
        pp_i <- p_i ^ 2
        spp_i <- sum(pp_i, na.rm = na.rm)
      } else {
        # Shannon's
        pp_i <- - p_i * log(p_i, base = log.base)
        spp_i <- sum(pp_i, na.rm = TRUE)
      }
